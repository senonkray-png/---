...
export default function GameScreen() {
  ...
  return (
    <div className="flex flex-col items-center min-h-dvh px-4 py-6">
      {/* Top Bar */}
      ...
      {/* Player Name */}
      <div
        className={`text-4xl font-bold mb-10 text-center transition-all ${
      ...
      {error && (
        <div className="w-full max-w-sm bg-red-900/30 border border-red-500/30 rounded-xl p-4 mb-4 text-center animate-fade-in">
          <p className="text-red-400 text-sm">{error}</p>
        </div>
      )}

      {questionText && !loading && (
        <div className="w-full max-w-sm flex-1 animate-fade-in">
          ...
          <div className="flex items-center justify-center gap-4 mb-6">
            <button
              ...
              className="flex items-center gap-2 bg-[var(--bg-card)] hover:bg-[var(--bg-secondary)] rounded-xl px-4 py-3 text-[var(--text-secondary)] transition-colors"
            >
          ...
          <button
            onClick={handleNext}
            className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-[var(--accent-pink)] to-[var(--accent-purple)] hover:opacity-90 rounded-xl px-4 py-4 text-lg font-bold transition-opacity"
          >
            Следующий ход
      ...
      {!questionText && !loading && (
        <div className="w-full max-w-sm flex-1 flex flex-col justify-center gap-4">
          <button
            ...
            className="w-full bg-gradient-to-r from-[var(--accent-purple)] to-purple-800 hover:opacity-90 rounded-2xl px-6 py-8 text-2xl font-bold transition-opacity active:scale-95 transform"
          >
            🔮 ПРАВДА
          </button>
          <button
            onClick={() => handleChoice('dare')}
            className="w-full bg-gradient-to-r from-[var(--accent-pink)] to-red-800 hover:opacity-90 rounded-2xl px-6 py-8 text-2xl font-bold transition-opacity active:scale-95 transform"
          >
            🔥 ДЕЙСТВИЕ
          </button>
        </div>
